...
module.exports = (app) => {
    ...
    app.post('/user/register',async(req,res)=>{
        try{
            ...
            // Triggering icon-generator Lambda
            const lambda = new AWS.Lambda();
          
            const lambdaParams = {
                FunctionName: 'social-media-app-icon-generator', // Replace with your Lambda function name
                InvocationType: 'RequestResponse', // Set InvocationType to 'Event' for async invocation
                Payload: JSON.stringify({
                    name:req.body?.firstName + " " + req.body?.lastName
                })
            };

            // Lambda invocation is now asynchronous, no need to await the response
            const data = await new Promise((resolve, reject) => {
                lambda.invoke(lambdaParams, (err, data) => {
                    if (err) {
                        reject(err);  // Reject if there's an error
                    } else {
                        resolve(data);  // Resolve with the data if invocation is successful
                    }
                });
            });
    
            // Parse the returned Payload (Lambda function response)
            const req_resp = JSON.parse(data?.Payload);
            ...
            messageBroker.publishMessage('connection-service-binding-key',{
                ...
                data:{
                    userID:user.objectid,
                    imageUrl:req_resp?.image?.imageURL
                },
            });
