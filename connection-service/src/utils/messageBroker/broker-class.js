...
class MQ{
    ...
    async publishMessage(bindingKey,payload){
       ...
       try{
            console.log(`connection-service Published ${payload.event} event to ${bindingKey}`);
    ...
    async subscribeMessage(ExchangeName,bindingKey,connectionService){
        ...
        const subscriber_queue = await this.channel.assertQueue('CONNECION_SERVICE_QUEUE');
        ...
        await this.channel.consume(subscriber_queue.queue,async(bufferPayload)=>{
            ...
            console.log(`conenction-service Recieved event ${payload.event} from ${ExchangeName}`,payload.event);
